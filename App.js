...
const modelJson = require('./assets/models5/model.json');
const modelWeights = [require('./assets/models5/group1-shard1of2.bin'), require('./assets/models5/group1-shard2of2.bin')];
...
const getModel = async () => {
  try {
    ...
    return await tf.loadGraphModel(
      // 'http://192.168.94.131:57857/model.json'
      bundleResourceIO(modelJson, modelWeights),

    );
  } catch (error) {
    console.error('Could not load model:', error.message);
...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      {!model && <Text style={styles.loading}>Loading...</Text>}
      {model === 'error' && <Text style={styles.error}>ERROR</Text>}
      {model && model !== 'error' && <TensorCamera
